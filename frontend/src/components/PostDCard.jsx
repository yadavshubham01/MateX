...
const Post = ({ project }) => {
  ...
  const handleLike = async () => {
    try {
      ...
      const response = await axios.post(
        `https://matex.onrender.com/api/projects/projects/${updatedProject.id}/like`,
        {},
        {
          headers: {
  ...
  const handleShare = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.post(
        `https://matex.onrender.com/api/projects/projects/${updatedProject.id}/share`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleJoin = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.post(
        `https://matex.onrender.com/api/projects/projects/${updatedProject.id}/join`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    <div className=" bg-black text-white shadow-md  p-4 mb-4 hover:bg-neutral-900 border-t-0 border-b border-r-0 border-l-0 border-[1px] border-neutral-600">
      ...
      <div className="grid grid-cols-4 pt-2 border-t border-b border-r-0 border-l-0 border-[1px] border-neutral-600">
        ...
        <div className="likes flex items-center mb-2">
          <button
            onClick={handleLike}
            className={`flex items-center justify-center w-10 h-10 p-2 transition-colors ${
        ...
        <div className="shares flex items-center mb-2">
          <button
            onClick={handleShare}
            className="px-1 py-1 text-white hover:rounded-full hover:text-blue-600 hover:bg-sky-200 bg-inherit cursor-pointer  "
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
        ...
        <div className="flex items-center mb-2">
          <button
            onClick={handleJoin}
            className="px-1 py-1 text-white hover:rounded-full hover:text-cyan-600 hover:bg-cyan-200"
          >
      ...
      <div className="mb-1 flex flex-row w-[100%] gap-5 mt-2">
        ...
        <div className="flex justify-center w-[15%]">
          <button
            ...
            className=" w-full h-[80%] bg-sky-500 text-white rounded hover:bg-sky-600"
          >
            Comment
          </button>
        </div>
